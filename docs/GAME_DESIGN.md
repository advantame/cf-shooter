...
画面下部の武器ボタンをスワイプして発射。クールタイム中は円形ゲージで残り時間を表示。

| 武器 | マーク | ダメージ | CT | 効果 |
|------|--------|----------|-----|------|
| グレネード | 六角形 | 40 | 5秒 | 減速しながら飛び、停止時に爆発 |
| ビーム | 菱形 | 60 | 8秒 | 0.75秒警告後に発射、太い直線レーザー |
| ショットガン | 三角形 | 8/発 | 3秒 | 親弾が垂直方向に子弾を発射し続ける |
| ミサイル | 星形 | 35 | 6秒 | 1秒間追尾後、直進 |
| シールド | 二重円 | - | 10秒 | 1.5秒間ダメージ無効 |

#### グレネード
- 初期速度: SIZE × 1.0 px/sec
- 速度が線形に減衰し、0.8秒後に速度0で爆発
- 爆発範囲: PLAYER_RADIUS × 8
- 飛距離は従来と同等

#### ビーム
- 発射コマンド後、0.75秒間は赤い点線が点滅して警告
- 警告後に実際にダメージ発生
- 射線上の全プレイヤーにダメージ（貫通）
- 太さ: SIZE × 0.04（従来の2倍）
- エフェクト表示: 0.3秒
...
#### ショットガン
- 親弾速度: SIZE × 0.6 px/sec（遅い）
- 親弾持続時間: 3秒
- 子弾発射間隔: 700ms
- 子弾速度: SIZE × 1.2 px/sec
- 子弾方向: 親弾の進行方向に対して垂直（両方向）
- 子弾ダメージ: 8

#### ミサイル
- 速度: SIZE × 1.125 px/sec（従来の75%）
- 旋回速度: 3.0 rad/sec
- 追尾時間: 最初の1秒間のみ、その後は直進
- 寿命: 3秒
...
- [ ] **アイテム**: 回復、スピードアップ
- [x] ~~**武器種追加**~~: 5種類の特殊武器を実装済み
- [x] ~~**スキル**~~: 特殊武器としてクールダウン付きで実装済み
